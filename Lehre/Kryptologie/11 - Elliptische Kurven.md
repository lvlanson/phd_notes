...
>Zum Zweck der Visualisierung haben wir für den Körper die reellen Zahlen $\mathbb{R}$ gewählt, damit die Addition geometrisch nachvollziehbar ist. Für die mathematische Disziplin der elliptischen Kurven sind vorwiegend die Körper über den rationalen Zahlen $\mathbb{Q}$ und der endlichen Mengen $\mathbb{K}_{q}$, wobei $q = p^k$ mit $k\in \mathbb{N}$
...
>>\end{align}$$
>>Für $x=0$ existiert demnach keine Quadratwurzel und ist somit kein Punkt auf der Kurve. Wir fahren mit $x=1$ fort und erhalten
>>$$\begin{align}
...
>$$nP := \underbrace{ P \oplus P \oplus \dots \oplus P }_{ n\text{-mal} }$$ 
...
>> 2. Bob wählt $m \in_{R} \mathbb{N}$ und sendet an Alice $$B= mG$$
