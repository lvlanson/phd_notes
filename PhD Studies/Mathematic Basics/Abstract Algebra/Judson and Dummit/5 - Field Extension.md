...
>
>Then **<span style="color:#38ffa9">there exists a unique [[7 - Polynomial Rings#^22fa43|irreducible]] [[7 - Polynomial Rings#^a2163f|monic]] polynomial $p(x) \in F[x]$ of smallest degree</span>** such that $$p(\alpha)=0$$ If $f(x)$ is another polynomial in $F[x]$ such that $f(\alpha)=0$, then $p(x)$ divides $f(x)$.
>>[!proof]
>> Let $$\phi_{\alpha}:F[x] \to E$$
>> denote the [[7 - Polynomial Rings#^4ffc7e|evaluation homomorphism]]