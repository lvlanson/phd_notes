...
>>[!proof] Proof can be seen ([[../../../../PDFs/judson2022.pdf#page=223|here]]), it is just some algebraic operations.

>[!lemma] Proposition Degree of Integral Domain Products ([[../../../../PDFs/judson2022.pdf#page=224|Source 1]], [[../../../../PDFs/dummit2003.pdf#page=249|Source 2]])
>Let $p(x), q(x) \in R[x]$ with $R$ being an [[1 - Algebraic Structures#^f48d09|integral domain]], then
>
>1. $\text{deg }p(x) + \text{deg }q(x) = \text{deg }\Big(p(x)q(x)\Big)$
>2. the units of $R[x]$ are just units of $R$
>3. $R[x]$ is an [[1 - Algebraic Structures#^f48d09|integral domain]].
>>[!proof]-
>>1. Let $p(x),q(x) \in R[x]$ be non-zero with
>>$$\begin{align}
...
>>
>>---
>>2. Let $p(x),q(x) \in R[x]$ be units, i.e.
>>$$p(x)q(x)=1$$
>>where $\text{deg}\Big(p(x)\Big) + \text{deg}\Big(q(x)\Big) = 0$. Therefore, $p(x), q(x)$ are constants in $R[X]$ and thus to be found in $R$.
>>---
>>3. It must hold $$p(x)\cdot q(x) = 0 \iff p(x) = 0 \lor q(x)=0$$
...
>>---
>>
>>We first show that the [[3 - Rings#^c3d051|ring homomorphism]] holds regarding [[#^b39226|addition]]
>>$$\begin{align}
