...
>[!theorem] Theorem Poincaré's Lemma
...
> \end{align}$$
> Now taking the second exterior derivative yields
> $$\begin{align}
...
>[!def] Definition Gradient, Curl and Divergence on $\mathbb{R}^3$
> Let $\mathcal{R}$ be a region in $\mathbb{R}^3$
>
> 1. Let $A$ be a smooth function on $\mathcal{R}$. Its **gradient** $\textbf{grad}(A)$ is the vector field on $\mathcal{R}$ given by $$\textbf{grad}(A) = A_x \boldsymbol{e}_{1} + A_y \boldsymbol{e}_{2} + A_z \boldsymbol{e}_{3}$$
> 2. Let $\boldsymbol{F} = A \boldsymbol{e}_{1}+  B \boldsymbol{e}_{2} + C \boldsymbol{e}_{3}$ be a smooth vector field on $\mathcal{R}$. Its **curl** is the smooth vector field on $\mathcal{R}$ given by $$\textbf{curl}(\boldsymbol{F}) = (C_y - B_z) \boldsymbol{e}_{1} + (A_z - C_x) \boldsymbol{e}_{2} + (B_x - A_y) \boldsymbol{e}_{3}$$
> 3. Let $\boldsymbol{F} = A  \boldsymbol{e}_{1} + B \boldsymbol{e}_{2}+ C \boldsymbol{e}_{3}$ be a smooth vector field on $\mathcal{R}$ given by $$\text{div}(\boldsymbol{F}) = A_x + B_y + C_z$$

^727a81

>[!def] Definition Fundamental Correspondence in $\mathbb{R}^3$
> Let $\mathcal{R}$ be a region in $\mathbb{R}^3$ and assume that all of the functions/vector fields below are smooth on $\mathcal{R}$.
> 1. The $0$-form $\phi=A$ corresponds to the function $A$.
> 1. The $1$-form $\phi = A \,dx +  B \, dy + C\, dz$ corresponds to the vector field $\boldsymbol{F} = A \boldsymbol{e}_{1} + B \boldsymbol{e}_{2} + C \boldsymbol{e}_{3}$.
> 3. The $2$-form $\phi = A \, dy \, dz + B \, dz \, dx + C \, dx \, dy$ corresponds to the vector field $\boldsymbol{F} = A \boldsymbol{e}_{1} + B \boldsymbol{e}_{2} + C \boldsymbol{e}_{3}$.
> 4. The $3$-form $\phi = A \, dx \, dy \, dz$ corresponds to the function $A$.
>
> In cases $(1)$ and $(4)$ we will write $\phi \leftrightarrow A$.
> In cases $(2)$ and $(3)$ we will write $\phi \leftrightarrow \boldsymbol{F}$.

>[!theorem] Theorem Fundamental Correspondence to Differential Forms
> Let $\longleftrightarrow$ denote the fundeamental correspondence and $d$ denote exterior differentiation. Then the following diagram commutes:
> $$\begin{align}
> 0  \text{-forms} &\longleftrightarrow \text{functions} \\
> d \downarrow \quad &\longleftrightarrow \quad \downarrow \text{grad} \\
> 1 \text{-form} &\longleftrightarrow \text{vector fields} \\
> d \downarrow \quad &\longleftrightarrow \quad \downarrow \text{curl} \\
> 2 \text{-form} &\longleftrightarrow \text{vector fields} \\
> d \downarrow \quad &\longleftrightarrow \quad \downarrow \text{curl} \\
> 3 \text{-form} &\longleftrightarrow \text{functions} \\
>\end{align}$$
> Here we assume that all functions, vector fields, and differential forms are smooth on some region $\mathcal{R}$ in $\mathbb{R}^3$.

>[!proof]-
> 1. Let $\phi = A$, the 0-form corresponding to the function $A$. By this [[#^3d96b3|proposition]] we know that the exterior derivative of $A$ on some region $\mathcal{R}$ in $\mathbb{R}^3$ is 
> $$d \phi = A_x \, dx + A_y \, dy + A_z \, dz$$
> which corresponds to the the [[#^727a81|gradient as defined]] on $\mathbb{R}^3$.
> 2. Let $\psi = A\, dx + B\, dy + C \, dz$, the 1-form corresponding to the vector field $\boldsymbol{F}$. By this [[#^3d96b3|proposition]] we have $$d \phi = (C_y - B_z) \, dy \, dz + (A_z - C_x) \, dz\, dx + (B_x - A_y)\, dx \, dy$$ which is by [[#^727a81|definition]] $\text{curl}(\boldsymbol{F})$.
> 3. Let $\psi = A\,dy\,dz + B\,dz\,dx + C \, dx\,dy$, the 2-form corresponding to the vector field $\boldsymbol{F}$. By this [[#^3d96b3|proposition]] we have $$d \phi = (A_x + B_y + C_z) \, dx \, dy \, dz$$
> which by [[#^727a81|definition]] on $\mathbb{R}^3$ corresponds to $\text{div}(\boldsymbol{F})$

>[!remark]
> 2. If the 0-form $\phi \longleftrightarrow$ the function $A$, then the 1-form $d \psi \longleftrightarrow$ the vector field $\textbf{grad}(A)$. 
> 3. If the 1-form $\phi \longleftrightarrow$ the vector field $\boldsymbol{F}$, then the 2-form $d \psi \longleftrightarrow$ the vector field $\textbf{curl}(F)$. 
> 4. If the 2-form $\phi \longleftrightarrow$ vector field $\boldsymbol{F}$, then the 3-form $d \psi \longleftrightarrow$ the function $\text{div}(\boldsymbol{F})$. 

>[!lemma] Corollary
> 1. For any smooth function $A$ on $\mathcal{R}$, $$\textbf{curl}\Big(\textbf{grad}(A)\Big) = 0$$
> 2. For any smooth vector field $\boldsymbol{F}$ on $\mathcal{R}$ $$\text{div}\Big(\textbf{curl}(\boldsymbol{F})\Big) = 0$$

>[!proof]-
> 1. Let $\phi \longleftrightarrow A$. Then $$\begin{align}
>  d \phi &\longleftrightarrow \textbf{grad}(A) \\
> d(d \phi) &\longleftrightarrow \textbf{curl}\Big(\textbf{grad}(A)\Big)
>\end{align}$$
> By Poincaré's lemma and the correspondence theorem we know that $$d(d \phi) = 0 \longleftrightarrow \textbf{curl}\Big(\textbf{grad}(A)\Big) = 0$$
> 2. Let $\phi \longleftrightarrow \boldsymbol{F}$. Then $$\begin{align}
>d \phi &\longleftrightarrow \textbf{curl}(A) \\
> d (d \phi) &\longleftrightarrow \text{div}\Big(\textbf{grad}(A)\Big)
>\end{align}$$
> By Poincaré's lemma and the correspondence theorem we know that $$d(d \psi) = 0 \longleftrightarrow \text{div}\Big(\textbf{curl}(A)\Big) = 0 \tag*{$\square$}$$

>[!remark]
> Leaving out some of the correspondences highlighted in the book, there is a more general correspondence on $\mathbb{R}^n$ and the hodge star operator, which are very straight forward.

## 1.4 The Converse of Poincaré's Lemma (I)

>[!remark] Reminder
> Note that the result of Poincaré's lemma states that $$d(d \phi) = 0$$
> i.e. that every exact form 
> - i.e. $\phi$ is exact if there exists a differential form $\psi$ such that $d \psi = \phi$ 
>
> is closed
> - i.e. $\phi$ is closed if $d \psi = 0$
> 
> The reverse is not unconditionally true.

>[!theorem] Converse of Poincaré's Lemma
> Let $\phi$ be a closed $k$-form defined on $\mathbb{R}^n, k>0$. Then $\phi$ is exact.

>[!proof] Proof by Induction
> 
> Preparation:
> - let $d_p$ denote the exterior differentiation with respect to **the first $p$ variables** $x_1, \ldots, x_p$ and consider $x_{p+1}, \ldots, x_n$ as constants, for example
> $$d_p(f) = f_{x_1}\, dx_1 + \ldots + f_{x_p} \, dx_p$$
>
> 1. Induction Hypothesis:
> - let $\phi$ be a $k$-form defined on $\mathbb{R}^n$ which involves only $d_{x_1}, \ldots, d_{x_p}$ with $p \in \{1, \ldots, n\}$ 
> - suppose that $d_p(\phi) = 0$ 
> - then there is a $(k-1)$-form $\psi$ with $\phi=d_p(\psi)$ 
> - furthermore, $\psi$ involves only $dx_1, \ldots, dx_{p-1}$ 
> - also, if for some $i > p$, $\phi$ does not involve the variables $x_i$, neither does $\psi$.
> 
> 2. Base Case $p = 1$
> For this case we know that there is only a single basis $1$-form, i.e.
> $$\phi = A(x_1, \ldots, x_n) \, dx_1$$
> We define the following function $$B(x_1, \ldots, x_n) = \int_{0}^{x_1} A(t, x_2, \ldots, x_n) \, dt$$
>
>>[!remark]- Verification of the Base Case
>> Let $\widetilde{A}$ denote an antiderivative of $A$ with respect to $x_1$. Integration gives
>> $$\begin{align}
>> B(x_1, ..., x_n) 
>> = \int_{0}^{x_1} A(t, x_2, ..., x_n) \, dt 
>> = \widetilde{A}(x_1, ..., x_n) - \widetilde{A}(0, ..., x_n).
>> \end{align}$$
>> We compute the following identity
>> $$\begin{align}
>> \frac{\partial B}{\partial x_1} 
>> &= \frac{\partial }{\partial x_1} 
>> \Big( \widetilde{A}(x_1, ..., x_n) 
>> - \widetilde{A}(0, ..., x_n) \Big).
>> \end{align}$$
>> Since $\widetilde{A}(0, \ldots, x_n)$ is independent of $x_1$, we have 
>> $$\begin{align}
>> \phantom{\frac{\partial B}{\partial x_1}}
>> &= A(x_1, ..., x_n) - 0 \\
>> &= A(x_1, ..., x_n).
>> \end{align}$$
>> By condition of the theorem, we know that 
>> $\phi = A(x_1, \ldots, x_n) \, dx_1$ is closed. Hence,
>> $$d\phi 
>> = d(A \, dx_1) 
>> = dA \, dx_1 
>> = \sum_{k=1}^{n} 
...
>> \, dx_k \, dx_1 
>> = 0.$$
>> Since $dx_1 \, dx_1 = 0$, the remaining terms
>> $$\sum_{k=2}^{n} 
>> \frac{\partial A}{\partial x_k} 
>> \, dx_k \, dx_1 = 0$$
>> imply that 
...
>> Hence $A$ depends only on $x_1$. Therefore $B$ depends only on $x_1$, and thus
>> $$\frac{\partial B}{\partial x_k} = 0 
>> \quad \text{for } 2 \leq k \leq n.$$
>> Consequently,
>> $$\begin{align}
>> d(B) 
>> &= \sum_{i=1}^{n} 
>> \frac{\partial B}{ \partial x_i} \, dx_i \\
>> &= \frac{\partial B}{\partial x_1} dx_1 \\
>> &= A(x_1, ..., x_n) \, dx_1.
>> \end{align}$$
>
> 3. Induction Step: 
> We will assume for the induction step, that the hypothesis is true for $p=m-1$. For the induction step we need some additional definitions:
> - let $d^m$ denote the exterior differentiation with respect to the variable $x_m$
>   - this means for a function $f$ we have $d^m(f) = f_{x_m} \, dx_{m}$
>   - it follows that $d_m = d_{m-1} + d^m$ which is $d_m (\alpha) = d_{m-1}(\alpha) + d^m(\alpha)$
>
> We will utilize the following observations:
> 1. for any $m$, $d_m(d_m(\alpha)) = 0$ (Poincaré's Lemma)
> 2. $d_m(\alpha) = 0$ and $\alpha$ only involves $d_{x_1}, \ldots, dx_{m-1} \implies d_{m-1}(\alpha) = 0 \land d^m(\alpha) = 0$
> 
>>[!remark]- Verification of the Second Observation
>> Note that $$d_m(\alpha) = d_{m-1}(\alpha) + d^m(\alpha)$$
>> The following properties verify the assertion
>> - $\alpha$ only contains $dx_1, \ldots, dx_{m-1}$
>> - $d_{m-1}(\alpha)$ only induces further $dx_1, \ldots, dx_{m-1}$ terms
>> - $d^m(\alpha)$ only induces further $dx_{m}$  terms
>> - the fact $d_m(\alpha) = 0$
>> 
>> We make the following conclusions:
>> 1. $\implies d^m(\alpha)$ does have $dx_m$ terms
>> 2. $\implies d_{m-1}(\alpha)$ does not have any $dx_m$ terms
>> 
>> From $(1)$ and $(2)$ $\implies$ to establish $d_m(\alpha) = d_{m-1}(\alpha) + d^m(\alpha) = 0$ either the terms have to cancel each other out or must be zero, but since the terms differ at least for $dx_m$ they cannot cancel out. In other words, the summands lie in different subspaces. Thus $$d_{m-1}(\alpha) = 0 \land d^m(\alpha) = 0$$
>
>4. Conclusion:
>   Suppose $\phi$ involves only $dx_1, \ldots, dx_m$ and $d_m(\phi) = 0$. We write
>   $$\phi = \phi_{m-1} + \phi^m$$
>   where 
>       - $\phi_{m-1}$ consists of all terms in $\phi$ which do not involves $dx_m$
>       - $\phi^m$ consists of terms which all involve $dx_m$
>   By definition of $\phi^m$
> $$\phi^m = dx_m (A^1\,dx_{I_1} + A^2\,dx_{I_2} + \ldots)$$
> We define as we did in the base case
> $$B^i = \int_{0}^{x_m} A^i(x_1, \ldots, x_{m-1}, t, x_{m+1} \ldots, x_n) \, dt$$
> By the same logic as in the base case, the fundamental theorem of calculus guarantees us, that $\frac{\partial B^i}{\partial x_m} = A^i$. We define now
> $$\psi^m = B^1 \, dx_{I_1} + B^2\, dx_{I_2} + \ldots$$
> Clearly,
> $$d^m(\psi^m) = \phi^m$$
> By this relation we require that $\phi^m$ does not involve some variable $x_i$ for $i>m$. We define
> $$\widetilde{\phi} = \phi - d_m(\phi^m)$$
> This construction implies that $\widetilde{\phi}^m$ does not contain any $dx_m$ term.
>>[!remark]- Verification
>> $$\begin{align}
>> \widetilde{\phi} = \phi - d_m(\psi^m) &= \psi - (d_{m-1} + d^m)(\psi^m) \\
>> &= \phi_{m-1} + \phi^m -(d_{m-1} + d^m)(\psi^m)\\
>> &= \phi_{m-1}+\phi^m - d_{m-1}(\psi^m) - d^m(\psi^m) \\
>> &= \phi_{m-1} - d_{m-1}(\psi^m)
>>\end{align}$$
>> Hence, none of the terms involve $dx_m$.
>
> - $\,$
> We know 
>$$\begin{align}
>d_m(\widetilde{\phi}) &= d_m(\phi - d_m(\psi^m)) \\
> &= d_m(\phi) - d_m(d_m(\psi)) \\
> &= 0
>\end{align}$$
> because 
> - $d_m(\phi) = 0$ follows from the assumption that $\phi$ is closed
> - $d_m(d_m(\psi))$ = 0 follows from Poincaré's lemma (observation (1))
> - because $d_m(\widetilde{\phi}) = 0$ and $\widetilde{\phi}$ involves only $dx_1, \ldots, dx_{m-1}$ it follows from observation 2 that $d_{m-1}(\widetilde{\phi}) = 0$
> - also from observation 2 it follows $d^m(\widetilde{\phi}) = 0$ 
> Since $\widetilde{\phi}$ does not involve any $dx_m$ terms, we can rewrite 
> $$\widetilde{\phi} = \widetilde{A}^1\, dx_{I_1} + \widetilde{A}^2 \, dx_{I_2} + \ldots$$
> such that none of the terms contain $dx_m$. Then by observation 2 
> $$0 = d^m(\widetilde{\phi}) = \widetilde{A}^1_{x_m} \, dx_m \, dx_{I_1} + \widetilde{A}^2_{x_m} \, dx_m \, dx_{I_2}+ \ldots$$
> Since none of the $dx_{I}$ contain $dx_m$ and are distinct, all of them must be $0$, i.e.
> $$\widetilde{A}^1_{x_m} = \widetilde{A}^2_{x_m} = \ldots = 0$$
> From this we can conclude that none of the terms include $x_m$, therefore we can apply the **induction hypothesis**, i.e.
> $$\widetilde{\phi} = d_{m-1}(\widetilde{\psi})$$
> where by induction hypothesis $\widetilde{\psi}$ involves only $dx_1, \ldots, dx_{m-2}$. But in fact since $\widetilde{\psi}$ does not contain any $x_m$ it actually involves $dx_1, \ldots, dx_{m-1}$, as $i = m > p=m-1$.
> Since $\widetilde{\psi}$ does not involve $x_m$, we have
> $$d^m(\widetilde{\psi}) = 0$$
> Therefore $$\begin{align}
>d_m(\widetilde{\psi}) &=  (d_{m-1} + d^m)(\widetilde{\psi}) \\
> &=d_{m-1}(\widetilde{\psi}) + d^m(\widetilde{\psi}) \\
> &=\widetilde{\phi} + 0 \\
> &= \widetilde{\phi}
>\end{align}$$
> We set $$\psi = \widetilde{\psi} + \psi^m$$
> and claim $\psi$ is the desired primitive. As highlighted, $\psi$ only involves $dx_1, \ldots, dx_{m-1}$ and if $\phi$ does not involve $x_i$ for $i>m$, then neither does $\psi$. We must verify that $d_m(\psi) = \phi$, thus
> $$\begin{align}
> d_m(\psi) &= d_m(\widetilde{\psi} + \psi^m) \\
> &= d_m(\widetilde{\psi}) + d_m(\psi^m) \\
> &= \widetilde{\phi} + \underbrace{d_m(\psi^m)}_{=0} \\
> &= \widetilde{\phi}
>\end{align}$$
