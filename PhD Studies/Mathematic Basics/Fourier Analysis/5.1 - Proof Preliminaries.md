...
>> &= e^{-iM\gamma x} \underbrace{ \big(1+ e^{-i\gamma x} + e^{-i2\gamma x} +\dots+e^{-i(N-M)\gamma x} \big) }_{ \text{partial geometric series} } \tag{1}
>>\end{align}$$
>>We identify a [[../Calculus/Series#^ef860a|partial geometric series]] with $a=1$, $r=e^{ix}$ and $n=N-M$.
>>$$1+ e^{-i\gamma x} + e^{-i2\gamma x} +\dots+e^{-i(N-M)\gamma x} = \frac{1-e^{-i(N-M+1)\gamma x}}{1-e^{-i\gamma x}}$$
...
>> &= e^{-iM\gamma x}\frac{1-e^{-i(N-M+1)\gamma x}}{1-e^{-i\gamma x}} \\
>> &= \frac{e^{-iM\gamma x}-e^{-i(N+1)\gamma x}}{1-e^{-i\gamma x}} \tag*{$\square$}
>>\end{align}$$
^bb8104
...
>$$\mathcal{FS}[t]|_{t}= \sum_{k=-\infty}^\infty c_{k}e^{i 2 \pi \omega_{k}t}$$
