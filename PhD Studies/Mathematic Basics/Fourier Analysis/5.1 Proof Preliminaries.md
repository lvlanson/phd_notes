...
>[!Lemma] Lemma Representation of Fourier Coefficients as Dirichlet Kernel ([[../../../PDFs/howell2016.pdf#page=189|Source]])
...
>> &= \frac{1}{p}\int _{-p / 2}^{p / 2} f(t_{0}+x)e^{-i2\pi \omega_{k}x}\, dx \\
>>\end{align}$$
...
>>&=  \int _{-p / 2}^{p / 2}  f(t_{0}+x)D_{M,N}(x)\, dx \\
>>\end{align}$$

>[!lemma] Lemma ([[../../../PDFs/howell2016.pdf#page=190|Source]])
...
>$$\sum_{k=M}^N c_{k}e^{i 2 \pi \omega_{k}t_{0}}= \int _{-p / 2}^{p / 2}  (f(t_{0}+x)-f(t_{0}))D_{M,N}(x)\, dx +f(t_{0})$$ 
...
>$$\sum_{k=M}^N c_{k}e^{i 2 \pi \omega_{k}t_{0}}= \int _{-p / 2}^{p / 2}  (f(t_{0}+x)-f_{0}(x))D_{-N,N}(x)\, dx + \frac{1}{2}(f_{0}^- + f_{0}^+)$$
>where
>$$f_{0}(x) = \begin{cases}
> f_{0}^- &  \text{ if } x < 0 \\
> f_{0}^+ & \text{ if } > 0 
